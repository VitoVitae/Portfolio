...
/**
 * \fn bool backtracking(tGrille grille, int numeroCase)
 * \brief Algotithme de backtracking
 * \param grille : grille de goku qui va être complétée
 * \param numeroCase : le numéro de la case a remplir
 * \return La fonction renvoie un booléen qui dit si il est possible de remplir la case, false si on ne peut pas, true sinon
 * 
 * 
 * 
*/
...
bool pileFull(pile p)
{
    return (p.summet==TAILLE*TAILLE);
}
...
bool iteratif_backtracking(tGrille grille){
    int numeroCase = 0, i = 0;
    ...
    while (numeroCase != TAILLE*TAILLE)
    {
        ligne = numeroCase / (TAILLE*1);
        colonne = numeroCase % (TAILLE*1);
        ...
        else
        {
            ...
            {
                if (possible(grille,ligne,colonne,i))
                {
